  ...
  export default {
    data () {
      return {
        ...
        fields: ['all','pid', 'locationName', 'latitude', 'longitude','address', 'phone'],
        progname:true,
      }
    },
    methods:{
      customFilter(items, search, filter) {
        ...
        if(selected=="all"){
           return items.filter(item => filter(item["pid"], search) || filter(item["locationName"], search) || filter(item["latitude"], search) || filter(item["longitude"], search) ||  
        filter(item["address"], search)|| filter(item["phone"], search));
        }
        else if(selected) {
